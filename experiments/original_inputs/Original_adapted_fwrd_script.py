...
def fit_ncp(par, spectrum, masses, data_x, data_y, data_e, fit_arguments):
    ...
    result = optimize.minimize(errfunc, par[:], args=(spectrum, masses, data_x, data_y, data_e),
                               method='SLSQP', bounds = boundaries, constraints=constraints)
    fitted_par = result["x"]
    ncp = calculate_ncp(fitted_par, spectrum , masses, data_x)
...
def calculate_ncp(par, spectrum , masses, data_x):
    ...
    for m in range(len(masses)):#   [parameter_index + number_of_mass * number_of_parameters_per_mass ]
        ...
        FSE =  - fun_derivative3(y,joy)*width**4/delta_Q * 0.72
...
def calculate_mean_widths_and_intensities(masses,widths,intensities,spectra, verbose):
    ...
    mean_widths,widths_std,mean_intensity_ratios,mean_intensity_ratios_std=np.zeros(
        (len(masses))),np.zeros(
        (len(masses))),np.zeros(
            (len(masses))),np.zeros(
                (len(masses)))
...
spectra_to_be_masked = spectra_to_be_masked[(spectra_to_be_masked >= first_spectrum) & (
    spectra_to_be_masked <= last_spectrum)]   #detectors within spectrums
...
CropWorkspace(
    ...
    OutputWorkspace=ws_name) #for MS correction
##spec_offset = mtd[ws_name].getSpectrum(0).getSpectrumNo()
...
for iteration in range(number_of_iterations):
    ...
    spectra, intensities, widths, positions, par_chi = block_fit_ncp(
        par,first_spectrum,last_spectrum, masses,ws_to_be_fitted, fit_arguments, verbose)
    # Calculate mean widths and intensities
    mean_widths, mean_intensity_ratios = calculate_mean_widths_and_intensities(
        masses, widths, intensities, spectra, verbose) # at present is not multiplying for 0,9
    ...
    if (number_of_iterations - iteration -1 > 0):
        ...
        # background workspace with ws_name :  str(ws_name)+"_gamma_background"
        ...
        Scale(
            InputWorkspace = str(ws_name)
            + "_gamma_background",
            OutputWorkspace = str(ws_name)
            + "_gamma_background",
            Factor=0.9,
            Operation = "Multiply")
        # evaluate multiple scattering correction --------- This creates a
        # background workspace with ws_name :  str(ws_name)+"_MulScattering"
        ...
        Minus(LHSWorkspace= ws_name+str(iteration+1),
              RHSWorkspace = str(ws_name)+"_MulScattering",
              OutputWorkspace = ws_name+str(iteration+1))
...
if fit_in_Y_space:
    ...
    for i, minimizer_sum in enumerate(['Levenberg-Marquardt','Simplex']):
        ...
        Fit(Function=function, InputWorkspace=ws_name+minimizer_sum+'_joy_sum_fitted',
            Output=ws_name+minimizer_sum+'_joy_sum_fitted',Minimizer=minimizer_sum)
...
##-------------------save results-------------------
savepath = currentPath / "original_results" / "4iter_forward_GB_MS.npz"
