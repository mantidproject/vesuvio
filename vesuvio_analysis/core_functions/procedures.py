...
def runPreProcToEstHRatio(bckwdIC, fwdIC):
    ...
    assert bckwdIC.runningSampleWS is False, "Preliminary procedure not suitable for Bootstrap."
...
def setInitFwdParsFromBackResults(bckwdScatResults, bckwdIC, fwdIC):
    ...
    if isHPresent(fwdIC.masses):
        ...
        assert len(backMeanWidths) == fwdIC.noOfMasses-1, "H Mass present, no of masses in frontneeds to be bigger" \
                                                          "than back by 1."
        ...
        assert len(backMeanWidths) == fwdIC.noOfMasses, "H Mass not present, no of masses needs to be the same for" \
