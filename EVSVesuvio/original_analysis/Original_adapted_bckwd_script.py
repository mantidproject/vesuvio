...
def load_ip_file(spectrum):
    ...
    ipfile = bckwdIC.InstrParsPath    #  ipFilesPath / 'ip2018_3.par'
    f = open(ipfile, 'r')
    data = f.read()
    lines = data.split('\n')
...
number_of_iterations = bckwdIC.noOfMSIterations + 1 #4              # This is the number of iterations for the reduction analysis in time-of-flight.
...
name = scriptName
first_spectrum, last_spectrum = bckwdIC.firstSpec, bckwdIC.lastSpec
...
if (name+'raw' not in mtd):
    ...
    Load(Filename=str(inputWSPath/name)+"_raw_backward.nxs", OutputWorkspace=name+'raw')
    Rebin(InputWorkspace=name+'raw',Params=bckwdIC.tofBinning,OutputWorkspace=name+'raw') 
...
if (name+'empty' not in mtd):
    ...
    Load(Filename=str(inputWSPath/name)+"_empty_backward.nxs", OutputWorkspace=name+'empty')
    Rebin(InputWorkspace=name+'empty',Params=bckwdIC.tofBinning,OutputWorkspace=name+'empty') 
...
transmission_guess = bckwdIC.transmission_guess
multiple_scattering_order = bckwdIC.multiple_scattering_order
number_of_events =  bckwdIC.number_of_events
...
hydrogen_to_mass0_ratio = bckwdIC.HToMassIdxRatio
# vertical_width, horizontal_width, thickness = 0.1, 0.1, 0.001 # expressed in meters
vertical_width = bckwdIC.vertical_width
horizontal_width = bckwdIC.horizontal_width
thickness = bckwdIC.thickness
create_slab_geometry(name,vertical_width, horizontal_width, thickness)

# masses = [12,16,27]
masses = [float(m) for m in bckwdIC.masses]
...
par = tuple(bckwdIC.initPars)
bounds = tuple(bckwdIC.bounds)
...
# constraints =  ()
constraints = bckwdIC.constraints
fit_arguments = [bounds, constraints]

################my edits##############
###################################

detectors_masked = np.array(bckwdIC.maskedSpecAllNo)
...
savepath = results_path / f"spec_{first_spectrum}-{last_spectrum}_iter_{number_of_iterations-1}_MS"
