...
#----------------------------------------------------------------------------------------
  ...
  def _fit_found_peaks(self, peak_table, peak_estimates_list, workspace_index, fit_output_name, xLimits=None):
    ...
    fit_result = Fit(Function=func_string, InputWorkspace=self._sample, IgnoreInvalidData=True, StartX=xmin, EndX=xmax,
...
#----------------------------------------------------------------------------------------
  ...
  def _fit_peaks(self):
    ...
    if self._shared_parameter_fit_type != "Individual":
      self._shared_parameter_fit(output_parameter_table_name, output_parameter_table_headers, x_range)

  def _shared_parameter_fit(self, output_parameter_table_name, output_parameter_table_headers, x_range):
    init_Gaussian_FWHM = read_fitting_result_table_column(output_parameter_table_name, 'f1.GaussianFWHM',
                                                          self._spec_list)
    init_Gaussian_FWHM = np.nanmean(init_Gaussian_FWHM[init_Gaussian_FWHM != 0])
    init_Lorentz_FWHM = read_fitting_result_table_column(output_parameter_table_name, 'f1.LorentzFWHM',
                                                         self._spec_list)
    ...
    peak_centres_errors = read_fitting_result_table_column(output_parameter_table_name, 'f1.LorentzPos_Err',
                                                           self._spec_list)
    invalid_spectra = identify_invalid_spectra(output_parameter_table_name, peak_centres, peak_centres_errors,
                                               self._spec_list)
    self._fit_shared_parameter(invalid_spectra, initial_params, output_parameter_table_headers,
                               output_parameter_table_name, x_range)
