...
from mvesuvio.system_tests.ncp_fits_inputs import (
...
mv.set_config(
    cache_directory=str((Path(__file__) / '..' / 'test_config').resolve()),
    experiment_id='test_expr',
    ip_folder=str((Path(__file__) / '..' / '..' / 'config' / 'ip_files').resolve())
)
...
class AnalysisRunner:
    ...
    @classmethod
    def _run(cls):
        # scattRes, yfitRes = mv.run(yes_to_all=True)
        ...
        AnalysisRunner._currentResults = forwardScatteringResults

    @classmethod
    def _load_benchmark_results(cls):
        testPath = Path(__file__).absolute().parent
        benchmarkResults = np.load(
            str(testPath / "stored_spec_144-182_iter_0_GC_MS.npz")
        )
...
class TestFitParameters(unittest.TestCase):
    ...
    def test_chi2(self):
        nptest.assert_almost_equal(self.orichi2, self.optchi2, decimal=7)

    def test_nit(self):
        nptest.assert_almost_equal(self.orinit, self.optnit, decimal=0)

    def test_intensities(self):
        nptest.assert_almost_equal(self.oriintensities, self.optintensities, decimal=3)
...
class TestNcp(unittest.TestCase):
    ...
    def test_ncp(self):
        ...
        nptest.assert_almost_equal(correctNansOri, self.optncp, decimal=6)
...
class TestFitWorkspaces(unittest.TestCase):
    ...
    def test_ws(self):
        nptest.assert_almost_equal(self.oriws, self.optws, decimal=10)
