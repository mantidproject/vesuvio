...
fwdIC = ai.ForwardInitialConditions(ipFilesPath)

inputWSPath = experimentsPath / "input_ws"
...
def load_ip_file(spectrum):
    ...
    ipfile = fwdIC.InstrParsPath    #  ipFilesPath / 'ip2018_3.par'
    f = open(ipfile, 'r')
    data = f.read()
    lines = data.split('\n')
...
number_of_iterations = fwdIC.noOfMSIterations + 1 #4              # This is the number of iterations for the reduction analysis in time-of-flight.
...
ws_name = scriptName
ws_name_raw = scriptName + "_raw_"
ws_name_empty = scriptName + "empty_"
first_spectrum, last_spectrum = fwdIC.firstSpec, fwdIC.lastSpec   # 144, 182  #144, 182

masses = list(fwdIC.masses) #[1.0079,12,16,27]
abs_cross_sections = [] # This should be a vector of absorprion-to-scattering cross sections for each mass.

simple_gaussian_fit = False # True

if load_data:
    spectrum_list=str(first_spectrum)+'-'+str(last_spectrum)
    # runs=wsFrontIC.runs #'43066-43076'
    # ipfile = fwdIC.ipfile#'./ip2018_3.par'
    ...
    Load(Filename= str(inputWSPath/scriptName)+"_raw_forward.nxs", OutputWorkspace=ws_name_raw)
    Rebin(InputWorkspace=ws_name_raw,Params=fwdIC.tofBinning, OutputWorkspace=ws_name_raw)
...
transmission_guess = fwdIC.transmission_guess #0.8537 #  experimental value from VesuvioTransmission
multiple_scattering_order = fwdIC.multiple_scattering_order 
number_of_events =  fwdIC.number_of_events   #2, 1.e5
vertical_width = fwdIC.vertical_width
horizontal_width = fwdIC.horizontal_width
thickness = fwdIC.thickness   #0.1, 0.1, 0.001 # expressed in meters
...
par = list(fwdIC.initPars)
bounds = list(fwdIC.bounds)

#constraints =  ({'type': 'eq', 'fun': lambda par:  par[6] -0.2963*par[3] })
constraints =  fwdIC.constraints #()
fit_arguments = [bounds, constraints]
# spectra to be masked
#spectra_to_be_masked = "173,174,179"
spectra_to_be_masked = list(fwdIC.maskedSpecAllNo) #[173, 174, 179]
H_spectra_to_be_masked = ""


MSCorrectionFlag = fwdIC.MSCorrectionFlag
GammaCorrectionFlag = fwdIC.GammaCorrectionFlag 
...
for iteration in range(number_of_iterations):
    ...
    CloneWorkspace(InputWorkspace=ws_name, OutputWorkspace="tmpCorrectionWs")
    ...
    if (number_of_iterations - iteration -1 > 0):
        if GammaCorrectionFlag:
            ...
            Minus(LHSWorkspace= "tmpCorrectionWs", RHSWorkspace = str(ws_name)+"_gamma_background", OutputWorkspace = "tmpCorrectionWs")
        ...
        if MSCorrectionFlag:
            ...
            Minus(LHSWorkspace= "tmpCorrectionWs", RHSWorkspace = str(ws_name)+"_MulScattering", OutputWorkspace = "tmpCorrectionWs")
        ...
        RenameWorkspace("tmpCorrectionWs", ws_name + str(iteration + 1))
...
##-------------------save results-------------------
results_path = experimentsPath / "original_results" 
results_path.mkdir(exist_ok=True)
saveName = f"spec_{first_spectrum}-{last_spectrum}_iter_{number_of_iterations-1}"
if GammaCorrectionFlag:
    saveName += '_GC'
if MSCorrectionFlag:
    saveName += '_MS'
savepath = results_path / saveName  #"4iter_forward_GB_MS.npz"
